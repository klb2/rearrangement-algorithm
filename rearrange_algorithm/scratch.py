...
def basic_rearrange(quant, prob, tol, tol_type, lookback, max_ra, optim_func, sample,
    ...
    while True:
        ...
        if tol_type == "absolute":
            _tol = abs(opt_rs_new - opt_rs_old)
        elif tol_type == "relative":
            _tol = abs((opt_rs_new - opt_rs_old)/opt_rs_old)
